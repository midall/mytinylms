...
public class ResourceElement extends AbstractElement {
    ...
    throws IOException, ParserConfigurationException, SAXException {
        ...
        if (! elem.getLocalName().equals("resource")) {
                throw new IOException("'" + getRequiredNamespace() + ":resource' element expected, but found '"+namespace+":"+elem.getTagName()+"' element.");
        }
        ...
        for (int i=0; i < nodes.getLength(); i++) {
            if (nodes.item(i) instanceof Element) {
                ...
                } else if (DOMs.isElement(child, namespace, "file")) {
                    FileElement file = new FileElement();
                    add(file);
                    this.fileList.add(file);
                    file.parse(child);
                } else if (DOMs.isElement(child, namespace, "dependency")) {
