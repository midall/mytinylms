...
public class OrganizationsElement extends AbstractElement {
    ...
    throws IOException, ParserConfigurationException, SAXException {
        ...
        if (! elem.getLocalName().equals("organizations")) {
                throw new IOException("'" + getRequiredNamespace() + ":organizations' element expected, but found '"+namespace+":"+elem.getTagName()+"' element.");
        }
        this.defaultOrganization = DOMs.getAttributeNS(elem, namespace, "default", null);
        ...
        for (int i=0; i < nodes.getLength(); i++) {
            if (nodes.item(i) instanceof Element) {
                ...
                if (DOMs.isElement(child, namespace, "organization")) {
    ...
    throws IOException {
        ...
        while (iter.hasNext()) {
            OrganizationElement elem = ((OrganizationElement) iter.next());
            if (elem.getIdentifier().equals(defaultOrganization)) {
                elem.exportToJavaScriptAsQuiz(out, depth + 1, gen, cm);
                break;
            }
            /*
            if (iter.hasNext()){
