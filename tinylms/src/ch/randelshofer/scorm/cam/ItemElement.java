/*
 * @(#)ItemElement.java 1.3  2009-09-01
 *
 ...
 * @version 1.3 2009-09-01 Support for quizzes added.
 ...
 */
public class ItemElement extends AbstractCAMElement {
    ...
    public void parse(Element elem)
        ...
        if (!elem.getLocalName().equals("item")) {
            throw new IOException("'" + getRequiredNamespace() + ":item' element expected, but found '" + namespace + ":" + elem.getTagName() + "' element.");
        }
        this.identifier = DOMs.getAttributeNS(elem, namespace, "identifier", null);
        this.identifierref = DOMs.getAttributeNS(elem, namespace, "identifierref", null);
        this.isVisible = DOMs.getAttributeNS(elem, namespace, "isvisible", "true").equals("true");
        this.parameters = DOMs.getAttributeNS(elem, namespace, "parameters", null);
        ...
        for (int i = 0; i < nodes.getLength(); i++) {
            if (nodes.item(i) instanceof Element) {
                ...
                } else if (DOMs.isElement(child, namespace, "datafromlms")) {
    ...
    public void exportToJavaScriptAsQuiz(PrintWriter out, int depth, IdentifierGenerator gen)
        ...
        if (itemList.size() == 0) {
            ...
            Iterator iter = itemList.iterator();
            while (iter.hasNext()) {
                ((ItemElement) iter.next()).exportToJavaScriptAsQuiz(out, depth, gen);
                if (iter.hasNext()) {
                    out.println(",");
                }
            }
    ...
    public String toString() {
        ...
        if (!isIdentifierLegal()) {
            buf.append(" <font color=blue><b>ILLEGAL ID</b></font>");
        }
