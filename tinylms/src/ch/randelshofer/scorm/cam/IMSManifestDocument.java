/*
 * @(#)IMSManifestDocument.java  1.3.1  2008-12-16
 *
 ...
 * @version 1.3.1 2008-12-16 Fixed encoding of baseURI.
 * <br>1.3 2006-10-10 Parse with XML namespaces.
...
public class IMSManifestDocument extends ManifestElement {
    ...
    public URI getManifestURI()  {
        ...
        try {
            if (pifFile != null) {
                uri = new URI("jar:"+pifFile.toURI()+"!/imsmanifest.xml");
            } else {
                uri = new URI(contentPackageFile.toURI()+"/imsmanifest.xml");
            }
    ...
    throws IOException {
        ...
        if (depth == 0) out.print("API.cam = ");
        out.println("new ManifestElement(\""+gen.getIdentifier(getIdentifier(), getIdentifier())+"\",\""+getVersion()+"\",");
        getOrganizationsElement().exportToJavaScriptAsQuiz(out, depth + 1, gen, cm);
        out.println();
        indent(out, depth + 1);
        out.println(",");
        getResourcesElement().exportToJavaScript(out, depth + 1, gen);
